...
let of_sexp s =
  ...
  List.iter t ~f:(function
    | Top_percent p | Top_percent_samples p ->
        let p = Percent.to_percentage p in
        if Float.O.(p < 0.0 || p > 1.0) then
          Report.user_error
            "Cannot take %fl percent of functions. Argument must be between \
             0%% and 100%%."
            p
...
let take_top_percent_samples l p =
  let total =
    List.fold l ~init:0L ~f:(fun acc (_, count) -> Execount.(acc + count))
  in
  ...
  let list, sum =
    List.fold l ~init:([], 0L) ~f:(fun (list, sum) (name, count) ->
        if Execount.compare sum max < 0 then
          ((name, count) :: list, Execount.(sum + count))
        else (list, sum))
  in
  if !verbose then
    printf
...
let apply_action l a =
  ...
  | Min_samples n ->
      List.take_while l ~f:(fun (_, count) ->
          Execount.(compare count (of_int n)) >= 0)
