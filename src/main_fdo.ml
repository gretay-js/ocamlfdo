...
let merge
  ...
  then (
    ...
    Report.logf
      ...
      files
      output_filename);
  if read_aggregated_perf_profile
  then A.Merge.merge_files files ~crc_config ~ignore_buildid ~output_filename
  else AD.Merge.merge_files files ~crc_config ~ignore_buildid ~output_filename
;;
...
let decode_file
  ...
  let aggr_perf_profile =
    ...
    then
      ...
      Profile.record ~accumulate:true "read_aggregated" A.read perf_profile_filename
    else (
      (* aggregate from perf.data *)
      let aggr_perf_profile =
        Profile.record_call ~accumulate:true "agg_perf_data" (fun () ->
            Perf_profile.read_and_aggregate
              perf_profile_filename
              binary_filename
              ignore_buildid
              expected_pids)
  ...
  then (
    ...
    Profile.record_call ~accumulate:true "linker_script_hot" (fun () ->
        Linker_script.write_hot filename agg_dec_profile ~reorder_functions ~check));
  ()
;;
...
let decode
  ...
  | _ ->
    ...
    merge
      tmp_files
      ~read_aggregated_perf_profile:false
      ~crc_config
      ~ignore_buildid
      ~output_filename
      ~report:false
    (* it just turns on the reporting, which would have been turned on earlier if needed. *);
    if write_linker_script_hot
    then (
...
let transform f ~algo ~extra_debug ~simplify_cfg ~alternatives =
  ...
  in
  let new_body = Profile.record ~accumulate:true "cfg_to_linear" CL.to_linear new_cfg in
...
let optimize
  ...
  let profile =
    ...
    | Some filename ->
      let agg =
        Profile.record ~accumulate:true "read_fdo_profile" AD.read_bin filename
      in
      Some agg
  in
  let algo =
  ...
  let process file =
    ...
    if Crcs.add_unit crcs ui ~hex:(Caml.Digest.to_hex crc) ~file
    then (
        ...
        List.map ui.items ~f:(fun item ->
            ...
            | Func f ->
              ...
              let transform ~alternatives =
                Func (transform f ~algo ~extra_debug ~simplify_cfg ~alternatives)
              in
              ...
              | exception Crcs.Near_match alternatives -> transform ~alternatives))
      in
      if !skipped > 0
      then
        Report.logf
...
let check files ~input =
  let files =
    ...
    | Some input ->
      ...
      then
        Printf.printf
          "Reading input files to check from %s, in addition to files listed on command \
           line"
          input;
  ...
  let check_item = function
    | Func f ->
      ...
      let fnew = { f with fun_body = new_body } in
      print_linear "After" fnew;
      if not (Check_invariants.equal_linear f new_body "")
      then (
  ...
  then (
    ...
    Profile.record_call ~accumulate:true "opt" (fun () ->
        optimize
          ...
          ~report
          ~simplify_cfg);
    Profile.record_call ~accumulate:true "phase II:emit" (fun () -> call_ocamlopt w Emit))
  else (
    if !verbose && (Option.is_some fdo_profile || extra_debug)
    then printf "Calling ocamlopt directly, without FDO.\n";
    call_ocamlopt w All)
;;
