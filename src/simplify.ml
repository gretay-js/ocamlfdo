...
(* CR gyorsh: Too complicated? This approach may be useful earlier in the
   compiler. Is there existing code in the compiler that does something
   similar? *)
...
module Rep = struct
  ...
  type rep = {eq: bit; lt: bit; gt: bit; uo: bit}

  type cint = Always | Cint of integer_comparison
...
(* Compute one comparison operator that is equivalent to the disjunction of
   the operators c1 and c2. [f] is the constructor to apply to the
   simplified comparison operator, unless the result is "Always" which is
   not expressible as a comparison. *)
let simplify_disjunction_int c1 c2 f =
  let open Rep in
  let res = cint_of_rep (disjunction (cint_to_rep c1) (cint_to_rep c2)) in
  match res with Cint c -> [f c] | Always -> [Cfg.Always]

let simplify_disjunction_float c1 c2 =
  let open Rep in
  let open Cfg in
  let res =
    cfloat_of_rep (disjunction (cfloat_to_rep c1) (cfloat_to_rep c2))
  in
  match res with
      ...
      if c1 = c2 then [Test c1]
        ...
        | Iinttest (Isigned cmp1), Iinttest (Isigned cmp2) ->
            simplify_disjunction_int cmp1 cmp2 (fun cmp ->
                Test (Iinttest (Isigned cmp)) )
        | Iinttest (Iunsigned cmp1), Iinttest (Iunsigned cmp2) ->
            simplify_disjunction_int cmp1 cmp2 (fun cmp ->
                Test (Iinttest (Iunsigned cmp)) )
        | Iinttest_imm (Isigned cmp1, n1), Iinttest_imm (Isigned cmp2, n2)
          when n1 = n2 ->
            simplify_disjunction_int cmp1 cmp2 (fun cmp ->
                Test (Iinttest_imm (Isigned cmp, n1)) )
        | ( Iinttest_imm (Iunsigned cmp1, n1)
          , Iinttest_imm (Iunsigned cmp2, n2) )
          when n1 = n2 ->
            simplify_disjunction_int cmp1 cmp2 (fun cmp ->
                Test (Iinttest_imm (Iunsigned cmp, n1)) )
