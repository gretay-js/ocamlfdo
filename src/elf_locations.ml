...
type t =
  ...
  ; mutable resolved_fun_intervals : function_sym Intervals.t
  ; mutable hits : int
  ; mutable misses : int
...
let create ~elf_executable =
  ...
  let map =
    Bigarray.array1_of_genarray
      (Unix.map_file fd Bigarray.int8_unsigned Bigarray.c_layout ~shared:false [| len |])
  in
  Unix.close fd;
...
let resolve_from_dwarf t ~f =
  ...
  | Some section ->
    ...
    let rec aux () =
      ...
      | None -> ()
      | Some (header, chunk) ->
        let check header (state : Owee_debug_line.state) (prev : l option) : l option =
          if state.end_sequence
          then None
          else (
            let filename = Owee_debug_line.get_filename header state in
            let cur = { filename; state = Owee_debug_line.copy state } in
            f cur prev;
            Some cur)
        in
...
let find_range t ~start ~finish ~fill_gaps ~with_inverse cur prev =
  ...
  | Some prev ->
    ...
    then (
      Printf.printf
        "find_range prev.state.filename=%s .addr=0x%x\n"
        prev.state.filename
        prev.state.address;
      Printf.printf
        "find_range cur.state.filename=%s .addr=0x%x\n"
        cur.state.filename
        cur.state.address);
    let filename =
      ...
      | None ->
        ...
        then
          Printf.printf
            "find_range at prev=0x%x: prev.filename=None, use prev.state.filename=%s\n"
            prev.state.address
            prev.state.filename;
        prev.state.filename
      | Some filename ->
        if !verbose then Printf.printf "find_range filename=%s\n" filename;
        filename
    in
    assert (prev.state.address <= cur.state.address);
    if start <= prev.state.address && finish >= prev.state.address
    then (
      let line = prev.state.line in
      if !verbose
      then Printf.printf "Caching loc 0x%x %s %d\n" prev.state.address filename line;
      ...
      then (
        let tbl =
          ...
          | None ->
            if !verbose
            then Printf.printf "creating a new func table for %s %d\n" filename line;
        ...
        | Some addr ->
          ...
          then
            Printf.printf
              "found entry in line table for %s %d: 0x%x\n"
              filename
              line
              addr);
      if fill_gaps
      then (
        let n = if cur.state.address < finish then cur.state.address else finish in
...
let find_all ~t:_ ~addresses cur prev =
  ...
  | Some prev ->
    ...
    then (
      Printf.printf
        "find_all prev.state.filename=%s .addr=0x%x\n"
        prev.state.filename
        prev.state.address;
      Printf.printf
        "find_all cur.state.filename=%s .addr=0x%x\n"
        cur.state.filename
        cur.state.address);
    let filename =
      match prev.filename with
    ...
    let rec loop pc =
      if Elf_addr.compare pc cur.state.address < 0
      then (
...
let resolve_all t addresses =
  ...
  if len > 0
  then (
    if !verbose then Printf.printf "resolve_all: input size=%d unique addresses\n" len;
    resolve_from_dwarf t ~f:(find_all ~t ~addresses);
    let resolved = Hashtbl.count addresses ~f:Option.is_some in
    assert (resolved <= len);
    if !verbose
    then Printf.printf "resolve_all: resolved %d out of %d addresses\n" resolved len)
;;
...
let resolve_function_containing t ~program_counter =
  ...
  | Some name ->
    ...
    (* Only cache in resolved_fun the pcs for which no name was found. The
         others have an entry in resolved_fun_cache cache. It takes longer to
         extract but uses less memory. *)
  ...
  | None ->
    ...
    | None ->
      ...
      let syms =
        Owee_elf.Symbol_table.functions_enclosing_address
          t.symtab
      ...
      then (
        Printf.printf
          "Found %d symbols for addr=0x%Lx\n"
          (List.length syms)
          program_counter;
        List.iter syms ~f:(fun sym ->
            ...
            let name =
              ...
              | Some name -> name
            in
            Printf.printf "start=0x%Lx finish=0x%Lx size=%Ld %s\n" start finish size name));
      let rec find_func syms =
        ...
        | sym :: tail ->
          ...
          (* Look for symbol that contains program counter. This is
                   ...
                   start+size of symbols, and sometimes previous symbol's end
                   of interval covers the start of the next symbol. This may
                   be a bug in Owee, or maybe intentional, but we can work
                   around it here. *)
          let open Int64 in
          if start = program_counter && size = 0L
          then (
            if !verbose
            then
              Printf.printf
                "Enclosing function is of size 0: start=0x%Lx finish=0x%Lx pc=0x%Lx\n"
                start
                finish
                program_counter;
            find_func tail)
          else if (size > 0L && start = program_counter)
                  ...
                  (* if size is non-zero, sometimes finish overlaps with the
                     start of the next function, so we check that the program
                     counter is strictly smaller. *)
          then (
            match Owee_elf.Symbol_table.Symbol.name sym t.strtab with
            | None ->
              if !verbose then Printf.printf "Symbol without a name at 0x%Lx\n" start;
              report "Caching fun " None program_counter;
              Hashtbl.add_exn t.resolved_fun ~key:addr ~data:None;
              None
            | Some name ->
              ...
              <- Intervals.insert t.resolved_fun_intervals fun_interval;
              Some fun_interval)
          else find_func tail
      in
      find_func syms)
...
let resolve_range t ~start ~finish ~with_inverse =
  ...
  then
    Printf.printf
      "Resolving function for cache: (0x%Lx,0x%Lx)%s\n"
      start
      finish
  ...
  resolve_from_dwarf
    ...
    ~f:
      (find_range
         t
         ~start:(Elf_addr.mk start)
         ~finish:(Elf_addr.mk finish)
         ~fill_gaps:false
         ~with_inverse);
  if !verbose
  then
    Printf.printf
      "inverse entries before=%d after=%d\n"
      oldlen
...
let is_function_symbol sym =
  ...
  | Func -> true
  | Notype | Object | Section | File | Common | TLS | GNU_ifunc | Other _ -> false
;;

let iter_symbols t ~func ~data ~f =
  Owee_elf.Symbol_table.iter t.symtab ~f:(fun s ->
      if Bool.equal func (is_function_symbol s)
